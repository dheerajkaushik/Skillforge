...
export const getQuizzesByLesson = (lessonId, token) =>
    axios.get(`${API_BASE}/api/student/lessons/${lessonId}/quizzes`, getHeaders(token));

export const submitQuiz = (quizId, answers, studentId, token) =>
    axios.post(`${API_BASE}/api/student/quizzes/${quizId}/submit?studentId=${studentId}`, answers, getHeaders(token));
...
export const getInstructorQuizzes = (lessonId, token) =>
    axios.get(`${API_BASE}/api/instructor/quizzes/lesson/${lessonId}`, getHeaders(token));

    // ✅ NEW: Get Single Quiz for Instructor
    export const getInstructorQuizById = (quizId, token) =>
        axios.get(`${API_BASE}/api/instructor/quizzes/${quizId}`, getHeaders(token));
...
export const addQuestion = (quizId, qData, token) => {
    return axios.post(
        `${API_BASE}/api/instructor/quizzes/${quizId}/questions`,
        null,
        {
            ...getHeaders(token),
            params: {
...
export const deleteQuiz = (quizId, token) =>
    axios.delete(`${API_BASE}/api/instructor/quizzes/${quizId}`, getHeaders(token));

export const deleteQuestion = (questionId, token) =>
    axios.delete(`${API_BASE}/api/instructor/questions/${questionId}`, getHeaders(token));
